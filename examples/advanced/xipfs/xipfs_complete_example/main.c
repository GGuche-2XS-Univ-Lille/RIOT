 ...
 *
 * @param mp The mountpoint to initialize.
 *
 * @return An integer < 0 on errors, >= 0 otherwise.
 */
...
static void init_mount_points(void)
{
    ...
    const size_t mps_count = sizeof(mps) / sizeof(mps[0]);
 ...
 *
 * @param src_filename The source filename to be copied.
 * @param dst_filename The destination filename.
 * @param is_executable_file File execution permission.
 *
 * @remarks noinline attribute is used here to prevent GCC from inlining this code into main function.
 * It would then reserve a calculated maximum amount of bytes on stack, and would never release it back
 * when calling shell_run.
 * This could lead to stack overflow/exhaustion in later shell commands.
 */
static __attribute__((noinline))
void copy_file(const char *src_filename, const char *dst_filename, uint32_t is_executable_file)
{
    ...
    if (xipfs_extended_driver_new_file(dst_filename, src_stat.st_size, is_executable_file) < 0) {
    ...
    while (true) {
        ...
        while (read_bytes_count > 0) {
            write_bytes_count = vfs_write(dst_fd, buffer + pos, read_bytes_count);
...
int main(void)
{
...

    {
        ...
        char *args[] = { (char *)memory_dumper_filename, "non-legit-ram", NULL };
        printf("%s in progress...\n", casename);
        if (xipfs_extended_driver_safe_execv(memory_dumper_filename, args) >= 0) {
    ...
    printf("Now, you can enter VFS shell commands to manage '/nvme0p0' and '/nvme0p1' mount points.\n");
